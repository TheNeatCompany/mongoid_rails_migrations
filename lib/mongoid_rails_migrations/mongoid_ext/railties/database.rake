namespace :db do
  namespace :mongo do
    ...
    task :drop => :environment do
      Mongoid.default_client.collections.each {|col| col.indexes.drop_all && col.drop unless ['system.indexes', 'system.users'].include?(col.name) }
    end
    ...
    unless Rake::Task.task_defined?("db:setup")
      ...
      task :setup => [ 'db:mongo:create', 'db:seed' ]
    end

    desc 'Delete data and seed'
    task :reseed => [ 'db:mongo:drop', 'db:seed' ]
    ...
    desc "Migrate the database through scripts in /db/migrate_mongodb. Target specific version with VERSION=x. Turn off output with VERBOSE=false."
    ...
    namespace :migrate do
      ...
      task :reset => ["db:mongo:drop", "db:create", "db:mongo:migrate"]
